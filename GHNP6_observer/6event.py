...
class Event:
    ...
    def __str__(self):
        if self.pos1 == None and self.pos2 == None:
            ...
            return f"Event({self.name}, (old list: {self.before_changes}, new list: {self.after_changes}, from position:{self.pos1}, to:{self.pos2} ))"
        else:
            return f"Event({self.name}, (old list: {self.before_changes}, new list: {self.after_changes}, on/from position:{self.pos1})"
